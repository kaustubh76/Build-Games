...
contract AIAgentRegistry is Ownable, ReentrancyGuard {
    ...
    // ============ Constants ============
    uint256 public constant MIN_STAKE_NOVICE = 0.1 ether;       // 0.1 CRwN (testnet)
    uint256 public constant MIN_STAKE_SKILLED = 0.5 ether;     // 0.5 CRwN (testnet)
    uint256 public constant MIN_STAKE_EXPERT = 2 ether;        // 2 CRwN (testnet)
    uint256 public constant MIN_STAKE_ORACLE = 10 ether;       // 10 CRwN (testnet)
    ...
    /**
     * @notice Authorize a contract to call recordCopyTradeFee
     */
    ...
    function recordCopyTradeFee(uint256 agentId, uint256 fee) external {
        if (msg.sender != owner() && !authorizedCallers[msg.sender]) {
            revert AIAgentRegistry__NotAgentOperator();
        }
