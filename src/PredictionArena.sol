...
contract PredictionArena is ReentrancyGuard {
    ...
    struct Battle {
        ...
        // Storage reference for battle data
        bytes32 battleDataHash;
    }

    struct Round {
        uint8 roundNumber;
        bytes32 warrior1ArgumentHash;  // Hash of argument stored on-chain
    ...
    // ============ Constants ============
    uint256 public constant MIN_STAKES = 0.01 ether;        // 0.01 CRwN minimum (testnet)
     ...
     * @param _w1ArgumentHash Hash of warrior 1's argument (stored off-chain)
    ...
    /**
     * @notice Store battle data hash (Storage reference)
     * @param _battleId Battle ID
     * @param _dataHash Storage root hash containing full battle data
     */
