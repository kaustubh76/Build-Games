...
describe('Event Tracking System', () => {
  ...
  describe('MirrorMarketCreated Event', () => {
    it('should create a new mirror market in database', async () => {
      ...
      expect(market?.onChainMarketId).toBe('123');
...
describe('RPC Client', () => {
  it('should connect to Avalanche Fuji testnet', async () => {
    const client = getPublicClient();
    const chainId = await client.getChainId();

    expect(chainId).toBe(43113); // Avalanche Fuji testnet
  });
  ...
  it('should get current block number', async () => {
    ...
    expect(blockNumber).toBeGreaterThan(0n); // Avalanche Fuji testnet
  });
});
