...
/**
 * Record trade to AIAgentRegistry on Avalanche Fuji Testnet
...
export function useRecordAgentTrade(): UseRecordAgentTradeResult {
  ...
  const registryAddress = chainsToContracts[AVALANCHE_CHAIN_ID]?.aiAgentRegistry as `0x${string}`;
  ...
  const recordTrade = useCallback(
    async (
      ...
      try {
        // Switch to Avalanche Fuji Testnet if needed
        if (currentChainId !== AVALANCHE_CHAIN_ID) {
          console.log('Switching to Avalanche Fuji Testnet for trade recording...');
          try {
            await switchChainAsync({ chainId: AVALANCHE_CHAIN_ID });
          } catch (switchError) {
            throw new Error(
              `Please switch to Avalanche Fuji Testnet (Chain ID: ${AVALANCHE_CHAIN_ID}) to record trades.`
            );
          }
        }
