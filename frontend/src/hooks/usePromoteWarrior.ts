...
export function usePromoteWarrior(): UsePromoteWarriorResult {
  ...
  const warriorsAddress = chainsToContracts[AVALANCHE_CHAIN_ID]?.warriorsNFT as `0x${string}`;
    ...
    async (tokenId: bigint): Promise<string> => {
      ...
      try {
        ...
        if (currentChainId !== AVALANCHE_CHAIN_ID) {
          console.log('Switching to Avalanche Fuji Testnet for warrior promotion...');
          try {
            await switchChainAsync({ chainId: AVALANCHE_CHAIN_ID });
          } catch (switchError) {
            throw new Error(
              `Please switch to Avalanche Fuji Testnet (Chain ID: ${AVALANCHE_CHAIN_ID}) to promote warriors.`
            );
          }
        }
