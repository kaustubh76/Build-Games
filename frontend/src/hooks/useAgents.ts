...
export function useAgents(options?: {
  ...
  const fetchAgents = useCallback(async (skipCache = false) => {
    ...
    try {
      ...
      // Only fetch iNFT agents from Avalanche chain (legacy registry agents deprecated)
...
/**
 * Fetch iNFT agents from Avalanche chain via API route
 * Uses server-side fetching to avoid browser CORS issues with RPC
 ...
 * Uses Avalanche Fuji Testnet via API route
 */
export function useFollowingAgents() {
  ...
  const fetchFollowing = useCallback(async () => {
    ...
    try {
      ...
      // Fetch following list from Avalanche chain via API
...
export function useAgentFollowers(agentId: bigint | null) {
  ...
  const fetchFollowers = useCallback(async () => {
    ...
    try {
      setLoading(true);
      // Fetch from Avalanche chain via agentINFTService (copy trading is on Avalanche)
 ...
 * Uses direct contract read from Avalanche chain for accurate, real-time status
 */
...
/**
 * Hook to fetch CRwN balance on Avalanche chain for iNFT staking
 *
 * Architecture:
 * - Avalanche Fuji (43113): iNFT minting, AI compute, storage
 */
export function useCRwNBalance() {
  const { address } = useAccount();
  const [balance, setBalance] = useState<bigint>(BigInt(0));
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchBalance = useCallback(async () => {
    ...
    try {
      ...
      // Fetch CRwN balance via API to avoid CORS issues
      const response = await fetch(`/api/crwn/balance?address=${address}`);
      const data = await response.json();
      ...
      console.error('Error fetching CRwN balance:', err);
