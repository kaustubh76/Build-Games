 ...
 *
 * All operations happen on Avalanche Fuji Testnet (Chain ID: 43113)
 * or Avalanche C-Chain Mainnet (Chain ID: 43114)
 */
...
// Chain ID constants - use Avalanche
...
/**
 * Mint a new iNFT
 * Note: This operation happens on Avalanche Fuji Testnet (Chain ID: 43113)
 */
export function useMintINFT(): UseMintINFTResult {
  ...
  const mint = useCallback(
    async (
      ...
      try {
        // Check if on correct chain (Avalanche Fuji)
        if (currentChainId !== AVALANCHE_CHAIN_ID) {
          console.log('Switching to Avalanche Fuji Testnet for iNFT mint...');
          try {
            await switchChainAsync({ chainId: AVALANCHE_CHAIN_ID });
          } catch (switchError) {
            throw new Error(
              `Please switch to Avalanche Fuji Testnet (Chain ID: ${AVALANCHE_CHAIN_ID}) to mint iNFTs. ` +
              `Add the network manually: RPC URL: https://api.avax-test.network/ext/bc/C/rpc, Chain ID: ${AVALANCHE_CHAIN_ID}`
            );
          }
        }

        // IPFS service using internal API route with fallback
        // Note: Avalanche network uploads can take 2-3 minutes due to blockchain confirmations
        const storageService = {
          uploadEncryptedMetadata: async (data: Uint8Array): Promise<string> => {
            ...
            // Try IPFS upload - no client timeout, server handles it
            // Avalanche uploads can take 2-3 minutes due to blockchain confirmation
            try {
              ...
              console.log('Uploading encrypted metadata to IPFS (this may take 2-3 minutes)...');
              ...
              if (result.transactionHash === 'pending-verification') {
                console.log('IPFS upload submitted (pending verification):', result.rootHash);
              } else if (result.transactionHash === 'existing') {
                console.log('File already exists in IPFS:', result.rootHash);
              } else {
              ...
              // This allows minting to proceed even if Avalanche storage is slow/unavailable
  ...
  const transfer = useCallback(
    async (tokenId: bigint, to: Address): Promise<string> => {
      ...
      try {
        // Check if on correct chain (Avalanche Fuji)
        if (currentChainId !== AVALANCHE_CHAIN_ID) {
          console.log('Switching to Avalanche Fuji Testnet for iNFT transfer...');
          try {
            await switchChainAsync({ chainId: AVALANCHE_CHAIN_ID });
          } catch (switchError) {
            throw new Error(
              `Please switch to Avalanche Fuji Testnet (Chain ID: ${AVALANCHE_CHAIN_ID}) to transfer iNFTs.`
            );
          }
        }
      ...
      try {
        ...
        if (currentChainId !== AVALANCHE_CHAIN_ID) {
          try {
            ...
            throw new Error(
              `Please switch to Avalanche Fuji Testnet (Chain ID: ${AVALANCHE_CHAIN_ID}) to authorize usage.`
            );
          }
        }
      ...
      try {
        ...
        if (currentChainId !== AVALANCHE_CHAIN_ID) {
          try {
            ...
            throw new Error(
              `Please switch to Avalanche Fuji Testnet (Chain ID: ${AVALANCHE_CHAIN_ID}) to revoke usage.`
            );
          }
        }
    ...
    const decrypt = async () => {
      ...
      try {
        ...
        if (metadataRef.startsWith('0g://')) {
          ...
          if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            throw new Error(errorData.error || `Failed to fetch from IPFS: ${response.status}`);
          }
          ...
          console.warn('Metadata stored locally (Avalanche unavailable during mint), cannot decrypt');
          setMetadata(null);
          return;
        } else {
...
/**
 * Mint CRwN tokens on Avalanche Fuji Testnet
 * Sends native Avalanche tokens to get equivalent CRwN (1:1 ratio)
 */
  ...
  const mintCRwN = useCallback(
    async (amount: string): Promise<string> => {
      ...
      try {
        // Check if on correct chain (Avalanche Fuji)
        if (currentChainId !== AVALANCHE_CHAIN_ID) {
          console.log('Switching to Avalanche Fuji Testnet for CRwN mint...');
          try {
            await switchChainAsync({ chainId: AVALANCHE_CHAIN_ID });
          } catch (switchError) {
            throw new Error(
              `Please switch to Avalanche Fuji Testnet (Chain ID: ${AVALANCHE_CHAIN_ID}) to mint CRwN tokens. ` +
              `Add the network manually: RPC URL: https://api.avax-test.network/ext/bc/C/rpc, Chain ID: ${AVALANCHE_CHAIN_ID}`
            );
          }
        }
        ...
        const hash = await walletClient.writeContract({
          ...
          functionName: 'mint',
          args: [amountWei],
          value: amountWei, // Send native Avalanche tokens
          account: address,
          chain: avalancheFuji,
        });
