 ...
 * - Reads copy trade configs from Avalanche Fuji Testnet (Chain ID: 43113)
 ...
 * Uses the /api/copy-trade/pnl endpoint which reads from Avalanche chain
 */
export function useCopyTradePnL(): UseCopyTradePnLResult {
  ...
  const calculatePnL = useCallback(async () => {
    ...
    try {
      // Call the API endpoint that reads from Avalanche chain
