  ...
  if (error) {
    return (
      <div className={`bg-gray-800/50 rounded-2xl border border-gray-700 p-8 ${className}`}>
        ...
        <button
          onClick={refetch}
          className="mt-4 mx-auto block px-4 py-2 bg-red-600 rounded-lg text-white"
        >
          Retry
        </button>
      </div>
...
function LeaderboardRow({ entry, rank, compact }: LeaderboardRowProps) {
  ...
  return (
    <div className={`grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-gray-700/30 transition-colors ${
      rank <= 3 ? 'bg-gradient-to-r from-red-900/10 to-transparent' : ''
    }`}>
      {/* Rank */}
      ...
      <div className="col-span-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-red-500/30 to-pink-500/30 rounded-full flex items-center justify-center border border-red-500/50">
