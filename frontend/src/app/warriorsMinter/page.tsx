...
// AI generation moved to API route
...
const WarriorsMinterPage = memo(function WarriorsMinterPage() {
  ...
  const generatePersonality = async () => {
    ...
    showMessage({
      ...
      text: "Hark! I call upon the ancient AI spirits to forge thy warrior's essence! This may take a moment, Chief...",
      duration: 5000
    });
    ...
    try {
      ...
      try {
        ...
        const bio = parsedResponse.bio || 'Generated by AI';
        ...
        setFormData({
          ...
          life_history: 'Generated by AI based on your character prompt',
      ...
      console.error("Error generating personality with AI:", error);
      
      // Fallback to show error message
      setFormData({
        name: 'Error - Try Again',
        bio: `Failed to generate character with AI: ${error instanceof Error ? error.message : 'Unknown error'}`,
  ...
  // Mint Warriors NFT function - uploads image and creates JSON metadata on storage
  const handleMintWarriorsNFT = async () => {
    ...
    try {
      // Upload image and create JSON metadata
      console.log('🚀 Uploading warrior data to storage...');
      ...
      console.log('📋 Encrypted URI (Metadata Root Hash):', encryptedURI);
  ...
  // Handle Warriors activation using AI
  const handleActivateWarriors = async (warriors: UserWarriors) => {
    ...
    try {
      ...
      // Call the AI service via API route
      console.log(`Activating ${warriors.name} using AI traits generator...`);
      ...
      console.log("AI Traits Generator Response:", response);
      ...
      console.error("Error activating Warriors with AI:", error);
  ...
  const WarriorsImage = memo(({ src, alt, className }: { src: string; alt: string; className: string }) => {
    ...
    const shouldUseRegularImg = (urlToCheck: string) => {
      ...
      // Always use regular img for storage root hashes
      if (urlToCheck.startsWith('0x')) {
        console.log(`🖼️ shouldUseRegularImg: storage root hash detected (${urlToCheck}), using regular img`);
        return true;
      }
    ...
    const [imageSrc, setImageSrc] = useState(() => {
      // Convert root hashes to proper URLs
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div className="pt-4">
                  ...
                  >
                    {isMinting ? 'UPLOADING TO STORAGE...' : (!isFormComplete ? 'COMPLETE ALL FIELDS' : 'MINT WARRIORS NFT')}
                  </button>
                  ...
                  {ipfsCid && (
                    <div className="mt-4 p-3 bg-green-900/30 border border-green-500 rounded-lg">
                      <p className="text-green-400 text-xs mb-2" style={{fontFamily: 'Press Start 2P, monospace'}}>
                        WARRIORS NFT METADATA UPLOADED
                      </p>
                      ...
                      <p className="text-yellow-300 text-xs mb-2" style={{fontFamily: 'Press Start 2P, monospace'}}>
                        ✅ IMAGE + JSON METADATA STORED ON AVALANCHE
                      </p>
                    </div>
                  )}
                </div>
              ...
              {connectedAddress && !chainsToContracts[chainId] && (
                <div className="mt-4 p-4 bg-yellow-900/50 border border-yellow-500 rounded-lg">
                  <p 
                    ...
                    style={{fontFamily: 'Press Start 2P, monospace'}}
                  >
                    CONTRACTS NOT DEPLOYED ON CHAIN {chainId}. PLEASE SWITCH TO AVALANCHE FUJI TESTNET (CHAIN 43113)
                  </p>
                </div>
              )}
            </div>
