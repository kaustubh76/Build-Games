...
const AI_AGENT_INFT = AVALANCHE_CONTRACTS.aiAgentINFT;
...
/**
 * Update agent performance on Avalanche when market resolves
 */
async function updateAgentPerformance(marketId: number, outcome: Outcome, privateKey: string) {
  try {
    ...
    // Setup Avalanche provider and wallet
    ...
    const aiAgentINFT = new ethers.Contract(AI_AGENT_INFT, AI_AGENT_INFT_ABI, avalancheWallet);
    ...
    for (const trade of trades) {
      try {
        ...
        // Record trade result on Avalanche
