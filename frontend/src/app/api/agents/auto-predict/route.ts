/**
 * API Route: Automatic Agent Prediction
 *
 * This endpoint triggers an iNFT agent to automatically:
 * 1. Fetch its encrypted strategy from IPFS
 * 2. Analyze market data using AI
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Get agent data from Avalanche
    const inftContract = new ethers.Contract(AVALANCHE_CONTRACTS.aiAgentINFT, AI_AGENT_INFT_ABI, avalancheProvider);
    ...
    // Get market data from Avalanche
    const marketContract = new ethers.Contract(AVALANCHE_CONTRACTS.predictionMarketAMM, PREDICTION_MARKET_ABI, avalancheProvider);
    ...
    try {
      ...
      console.warn('Could not fetch agent metadata from storage:', err);
    }
