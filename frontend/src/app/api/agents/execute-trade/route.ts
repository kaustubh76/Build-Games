 ...
 *
 * Security:
 * - Only executes verified predictions from AI Compute
...
// AIAgentINFT ABI for recording trades (on Avalanche)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!prediction.isVerified) {
      throw ErrorResponses.badRequest('Cannot execute unverified prediction. Only verified predictions are allowed.');
    }
    ...
    const crownContract = new ethers.Contract(AVALANCHE_CONTRACTS.crownToken, CROWN_ABI, wallet);
    const marketContract = new ethers.Contract(AVALANCHE_CONTRACTS.predictionMarketAMM, MARKET_ABI, wallet);
    ...
    const allowance = await crownContract.allowance(wallet.address, AVALANCHE_CONTRACTS.predictionMarketAMM);
    if (allowance < tradeAmount) {
      console.log(`   Approving ${ethers.formatEther(tradeAmount)} CRwN for PredictionMarket...`);
      const approveTx = await crownContract.approve(AVALANCHE_CONTRACTS.predictionMarketAMM, tradeAmount);
    ...
    // Record trade to AIAgentINFT on Avalanche for trade history
    let recordTradeResult = null;
    try {
      console.log(`   📝 Recording trade on AIAgentINFT (Avalanche)...`);
      ...
      console.log(`   ✅ Trade recorded on Avalanche: ${recordReceipt.hash}`);
    } catch (recordError) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      rateLimit,
      contracts: {
        crownToken: AVALANCHE_CONTRACTS.crownToken,
        predictionMarket: AVALANCHE_CONTRACTS.predictionMarketAMM,
        rpc: AVALANCHE_RPC
      }
    });
  } catch (error) {
