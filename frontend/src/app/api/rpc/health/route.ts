...
// Avalanche RPC URLs for health checks
const AVALANCHE_RPC_URLS = {
  primary: getAvalancheRpcUrl(),
  fallback: getAvalancheFallbackRpcUrl(),
};
...
async function testRPCEndpoint(url: string): Promise<RPCHealthResult> {
  ...
  try {
    ...
    // Verify it's Avalanche (Fuji: 43113 = 0xa869, Mainnet: 43114 = 0xa86a)
    const isCorrectChain = chainId === '0xa869' || chainId === '0xa86a';

    if (!isCorrectChain) {
      return {
        url,
        status: 'unhealthy',
        error: `Wrong chain ID: ${chainId} (expected 0xa869 for Avalanche Fuji or 0xa86a for Avalanche Mainnet)`,
        latency,
      };
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get RPC URLs
    const primaryUrl = AVALANCHE_RPC_URLS.primary;
    const fallbackUrl = AVALANCHE_RPC_URLS.fallback;
