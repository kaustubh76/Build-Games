...
export class ExternalMarketEventListener {
  ...
  constructor(rpcUrl?: string) {
    this.client = createPublicClient({
      chain: avalancheFuji,
      transport: http(rpcUrl || getAvalancheRpcUrl()),
    });
  }
  ...
  async start(fromBlock?: bigint): Promise<void> {
    ...
    // Poll for new events every 2 seconds (Avalanche block time)
  ...
  private async pollEvents(): Promise<void> {
    try {
      ...
      ChainMetrics.setEventsSynced(Number(this.lastProcessedBlock));
  ...
  private async processLog(log: Log): Promise<void> {
    ...
    try {
      ...
      ChainMetrics.recordEventProcessed('unknown', false);
  ...
  private async handleMirrorMarketCreated(log: Log): Promise<void> {
    try {
      ...
      const event: MirrorMarketCreatedEvent = {
        ...
        onChainMarketId: args.marketId || 0n,
