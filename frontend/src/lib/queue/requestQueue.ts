...
import { ChainMetrics } from '../metrics';
...
export class RequestQueue<T = any> {
  ...
  async enqueue<R>(
    ...
    if (this.queue.length >= this.config.maxSize) {
      ...
      ChainMetrics.incrementCounter('queue_items_dropped_total');
    ...
    return new Promise<R>((resolve, reject) => {
      ...
      ChainMetrics.incrementCounter('queue_items_added_total', 1, {
        priority: QueuePriority[item.priority],
      });
      ChainMetrics.setGauge('queue_size', this.queue.length);
  ...
  private async startProcessing<R>(
    ...
    while (this.queue.length > 0 || this.processing.size > 0) {
      ...
      while (
        ...
        ChainMetrics.setGauge('queue_size', this.queue.length);
        ChainMetrics.setGauge('queue_processing', this.processing.size);
  ...
  private async processItem<R>(
    ...
    try {
      ...
      ChainMetrics.observeHistogram('queue_item_wait_time_ms', waitTime);
      ...
      if (item.retries < item.maxRetries) {
        ...
        ChainMetrics.incrementCounter('queue_items_retried_total');
        return;
      }
  ...
  private async onItemComplete(item: QueueItem<T>): Promise<void> {
    ...
    ChainMetrics.incrementCounter('queue_items_completed_total', 1, {
      priority: QueuePriority[item.priority],
    });
  }
  ...
  private async onItemFailed(item: QueueItem<T>, error: Error): Promise<void> {
    this.metrics.failed++;
    ChainMetrics.incrementCounter('queue_items_failed_total', 1, {
      priority: QueuePriority[item.priority],
      error: error.message,
    });
  ...
  clear(): void {
    ...
    ChainMetrics.incrementCounter('queue_items_dropped_total', dropped);
    ChainMetrics.setGauge('queue_size', 0);
