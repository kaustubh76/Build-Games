...
export class ChainMetrics {
  ...
  static recordEventProcessed(eventName: string, success: boolean): void {
    this.registry.incrementCounter('chain_events_processed_total', 1, {
      event: eventName,
  ...
  static recordEventProcessingTime(eventName: string, durationMs: number): void {
    this.registry.observeHistogram('chain_event_processing_duration_ms', durationMs, {
      event: eventName,
    });
  }

  static setEventsSynced(blockNumber: number): void {
    this.registry.setGauge('chain_events_synced_block', blockNumber);
  }

  static setBlocksBehind(blocks: number): void {
    this.registry.setGauge('chain_blocks_behind', blocks);
  }

  // ========== RPC Metrics ==========

  static recordRPCCall(endpoint: string, method: string, success: boolean, durationMs: number): void {
    this.registry.incrementCounter('chain_rpc_calls_total', 1, {
      endpoint,
      method,
      status: success ? 'success' : 'error',
    });

    this.registry.observeHistogram('chain_rpc_call_duration_ms', durationMs, {
      endpoint,
      method,
    });
  }
  ...
  static recordRPCError(endpoint: string, errorType: string): void {
    this.registry.incrementCounter('chain_rpc_errors_total', 1, {
      endpoint,
  ...
  static setRPCCircuitBreakerState(endpoint: string, state: string): void {
    this.registry.setGauge('chain_rpc_circuit_breaker_state', state === 'OPEN' ? 1 : 0, {
      endpoint,
    });
  }
  ...
  static recordDatabaseQuery(operation: string, table: string, durationMs: number): void {
    this.registry.incrementCounter('chain_database_queries_total', 1, {
      operation,
      table,
    });

    this.registry.observeHistogram('chain_database_query_duration_ms', durationMs, {
      operation,
      table,
    });
  }
  ...
  static recordDatabaseError(operation: string, table: string, errorType: string): void {
    this.registry.incrementCounter('chain_database_errors_total', 1, {
      operation,
      table,
      error_type: errorType,
    });
  ...
  static setTotalMarkets(count: number): void {
    this.registry.setGauge('chain_total_markets', count);
  }

  static setActiveMarkets(count: number): void {
    this.registry.setGauge('chain_active_markets', count);
  }

  static setTotalTrades(count: number): void {
    this.registry.setGauge('chain_total_trades', count);
  }

  static recordTradeVolume(amount: string): void {
    const volumeNumber = parseFloat(amount);
    if (!isNaN(volumeNumber)) {
      this.registry.incrementGauge('chain_total_volume', volumeNumber);
    }
  }

  static recordMarketCreated(source: string): void {
    this.registry.incrementCounter('chain_markets_created_total', 1, { source });
  }

  static recordTradeExecuted(source: string, direction: string): void {
    this.registry.incrementCounter('chain_trades_executed_total', 1, {
      source,
      direction,
    });
  }
  ...
  static recordMarketResolved(source: string, outcome: string): void {
    this.registry.incrementCounter('chain_markets_resolved_total', 1, {
      source,
      outcome,
    });
  }
  ...
  static recordOracleOperation(operation: string, success: boolean): void {
    this.registry.incrementCounter('chain_oracle_operations_total', 1, {
      operation,
      status: success ? 'success' : 'error',
    });
  }
  ...
  static recordOracleVerificationTime(durationMs: number): void {
    this.registry.observeHistogram('chain_oracle_verification_duration_ms', durationMs);
  }

  // ========== Error Recovery Metrics ==========

  static recordRetryAttempt(operation: string, attempt: number): void {
    this.registry.incrementCounter('chain_retry_attempts_total', 1, {
      operation,
  ...
  static recordFailedOperation(operation: string, severity: string): void {
    this.registry.incrementCounter('chain_failed_operations_total', 1, {
      operation,
      severity,
    });
  }
...
export class PerformanceTimer {
  ...
  constructor(operation: string) {
    ...
    ChainMetrics.recordEventProcessingTime(this.operation, duration);
    return duration;
  }
...
// Export singleton
export const chainMetrics = ChainMetrics;
