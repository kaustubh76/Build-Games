...
import { chainsToContracts, crownTokenAbi, getChainId, getAvalancheRpcUrl } from '../constants';
...
// Avalanche Chain configuration for iNFT operations
const AVALANCHE_CHAIN_ID = getChainId(); // 43113
const AVALANCHE_RPC_URL = getAvalancheRpcUrl();

// Create a dedicated Avalanche public client for iNFT reads
const avalanchePublicClient = createPublicClient({
  chain: avalancheFuji,
  transport: http(AVALANCHE_RPC_URL),
});

// Helper function to read from Avalanche chain
async function readContractOnAvalanche<T>(params: {
  address: Address;
  abi: any;
  functionName: string;
...
class AIAgentINFTService {
  ...
  private chainId: number = AVALANCHE_CHAIN_ID; // Use Avalanche chain for iNFT
  ...
  /**
   * Get the chain ID used for iNFT operations (Avalanche Fuji)
   */
  getChainId(): number {
    return this.chainId;
  }
  ...
  /**
   * Get the Avalanche RPC URL for wallet configuration
   */
  getAvalancheRpcUrl(): string {
  ...
  /**
   * Get owner of token (reads from Avalanche chain)
   */
  async getOwner(tokenId: bigint): Promise<Address | null> {
    try {
      const owner = await readContractOnAvalanche<Address>({
  ...
  /**
   * Get encrypted metadata reference (reads from Avalanche chain)
   */
  async getEncryptedMetadataRef(tokenId: bigint): Promise<string | null> {
    try {
      const ref = await readContractOnAvalanche<string>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  /**
   * Get metadata hash (reads from Avalanche chain)
   */
  async getMetadataHash(tokenId: bigint): Promise<`0x${string}` | null> {
    try {
      const hash = await readContractOnAvalanche<`0x${string}`>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  /**
   * Get agent on-chain data (reads from Avalanche chain)
   */
  async getAgentData(tokenId: bigint): Promise<AgentOnChainData | null> {
    try {
      const data = await readContractOnAvalanche<AgentOnChainData>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  /**
   * Get agent performance (reads from Avalanche chain)
   */
  async getAgentPerformance(tokenId: bigint): Promise<AgentINFTPerformance | null> {
    try {
      const perf = await readContractOnAvalanche<AgentINFTPerformance>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  /**
   * Get pending transfer (reads from Avalanche chain)
   */
  async getPendingTransfer(tokenId: bigint): Promise<PendingTransfer | null> {
    try {
      const transfer = await readContractOnAvalanche<PendingTransfer>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  /**
   * Get authorization for an executor (reads from Avalanche chain)
   */
  async getAuthorization(tokenId: bigint, executor: Address): Promise<Authorization | null> {
    try {
      const auth = await readContractOnAvalanche<Authorization>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  /**
   * Check if executor is authorized (reads from Avalanche chain)
   */
  async isAuthorizedExecutor(tokenId: bigint, executor: Address): Promise<boolean> {
    try {
      const result = await readContractOnAvalanche<boolean>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  /**
   * Get total supply (reads from Avalanche chain)
   */
  async getTotalSupply(): Promise<bigint> {
    try {
      const supply = await readContractOnAvalanche<bigint>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  /**
   * Get tokens owned by address (reads from Avalanche chain)
   */
  async getTokensOfOwner(owner: Address): Promise<bigint[]> {
    try {
      const balance = await readContractOnAvalanche<bigint>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
      ...
      for (let i = 0; i < Number(balance); i++) {
        const tokenId = await readContractOnAvalanche<bigint>({
          address: this.contractAddress,
          abi: AIAgentINFTAbi,
   ...
   * IMPORTANT: This function operates on Avalanche Fuji Testnet (Chain ID: 43113)
   */
  async mintINFT(
  ...
  /**
   * Transfer iNFT with re-encryption via TEE Oracle
  ...
  async transferINFTWithReEncryption(
    ...
    // Request re-encryption proof from TEE Oracle
  ...
  async getCopyTradeConfig(user: Address, tokenId: bigint): Promise<{
    ...
    try {
      const config = await readContractOnAvalanche<[bigint, bigint, bigint, bigint, boolean]>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  async getUserFollowing(user: Address): Promise<bigint[]> {
    ...
    try {
      const following = await readContractOnAvalanche<readonly bigint[]>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  async getAgentFollowers(tokenId: bigint): Promise<Address[]> {
    ...
    try {
      const followers = await readContractOnAvalanche<readonly Address[]>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
  ...
  async getFollowerCount(tokenId: bigint): Promise<number> {
    ...
    try {
      const count = await readContractOnAvalanche<bigint>({
        address: this.contractAddress,
        abi: AIAgentINFTAbi,
