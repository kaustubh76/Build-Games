 ...
 *
 * This service handles AI agent iNFTs and mirror markets on Avalanche chain
...
const AVALANCHE_CHAIN = {
  ...
  rpcUrls: {
    default: { http: [getAvalancheRpcUrl()] },
    public: { http: [getAvalancheRpcUrl()] },
  },
} as const;
...
class ExternalMarketAgentService {
  private avalanchePublicClient = createPublicClient({
  ...
  private async executeWithFallback<T>(
    operation: (client: typeof this.avalanchePublicClient) => Promise<T>
  ): Promise<T> {
    try {
      return await operation(this.avalanchePublicClient);
    } catch (error) {
      if (this.isTimeoutError(error)) {
        console.warn('[ExternalMarketAgent] Avalanche primary RPC timed out, trying fallback...');
  ...
  async canAgentTrade(agentId: bigint, source: MarketSource): Promise<boolean> {
    try {
      ...
      const enabled = await this.avalanchePublicClient.readContract({
  ...
  async isAgentActive(agentId: bigint): Promise<boolean> {
    try {
      const active = await this.avalanchePublicClient.readContract({
        address: this.aiAgentINFTAddress,
        abi: AIAgentINFTAbi,
  ...
  async getAgentExternalPerformance(
    ...
    try {
      const [stats, tradeHistory] = await Promise.all([
        this.avalanchePublicClient.readContract({
          address: this.aiAgentINFTAddress,
          abi: AIAgentINFTAbi,
