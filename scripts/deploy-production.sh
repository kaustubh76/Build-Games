...
#
# Automated deployment script for Warriors AI Avalanche implementation
#
# Usage:
...
setup_systemd_services() {
    ...
    if [ -f "$PROJECT_ROOT/deployment/avalanche-event-listener.service" ]; then
        execute_command \
            "sudo cp $PROJECT_ROOT/deployment/avalanche-event-listener.service /etc/systemd/system/" \
            "Install avalanche-event-listener service"
    fi
    ...
    execute_command \
        "sudo systemctl enable avalanche-event-listener" \
        "Enable avalanche-event-listener service"
}
...
restart_services() {
    ...
    execute_command \
        "sudo systemctl stop avalanche-event-listener || true" \
        "Stop avalanche-event-listener"
    ...
    execute_command \
        "sudo systemctl start avalanche-event-listener" \
        "Start avalanche-event-listener"
    ...
    if sudo systemctl is-active --quiet avalanche-event-listener; then
        print_success "avalanche-event-listener is running"
    else
        print_error "avalanche-event-listener failed to start"
        print_info "Check logs: sudo journalctl -u avalanche-event-listener -n 50"
        return 1
    fi
}
...
print_deployment_summary() {
    ...
    sudo systemctl status avalanche-event-listener --no-pager | grep "Active:" || true
    ...
    echo "  Event Listener: sudo journalctl -u avalanche-event-listener -f"
    ...
    echo -e "\n${BOLD}Management:${NC}"
    echo "  Restart:        sudo systemctl restart warriors-app avalanche-event-listener"
    echo "  Stop:           sudo systemctl stop warriors-app avalanche-event-listener"
    echo "  Status:         sudo systemctl status warriors-app avalanche-event-listener"
...
main() {
    ...
    echo -e "${BOLD}Project:${NC} Warriors AI Avalanche Implementation"
